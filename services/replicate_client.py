...
def _pick_first_url(output: Any) -> Optional[str]:
    # Replicate outputs vary: could be a URL string, list of URLs, or dict with urls.
    if output is None:
        return None
    if isinstance(output, str):
        return output
    if isinstance(output, list) and output:
        # sometimes list contains strings or objects with 'url'
        first = output[0]
...
async def run_nano_banana(
    *,
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            # First attempt: include aspect ratio if requested
            include_ar = True
            output = await asyncio.wait_for(
                asyncio.to_thread(replicate.run, MODEL, input=_build_inputs(include_ar)),
            ...
            if aspect_ratio and any(k in msg for k in ("aspect_ratio", "unexpected", "invalid")):
                try:
                    output = await asyncio.wait_for(
                        asyncio.to_thread(replicate.run, MODEL, input=_build_inputs(False)),
                        timeout=_REQUEST_TIMEOUT,
                    )
                    url = _pick_first_url(output)
                    if not url:
...
async def edit_image(images: list[bytes], prompt: str, aspect_ratio: Optional[str] = None) -> Optional[bytes]:
    return await run_nano_banana(prompt=prompt, images=images, aspect_ratio=aspect_ratio)


async def generate_image(prompt: str, aspect_ratio: Optional[str] = None) -> Optional[bytes]:
    return await run_nano_banana(prompt=prompt, images=[], aspect_ratio=aspect_ratio)
