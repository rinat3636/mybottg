...
from services.replicate_client import edit_image, generate_image
...
async def _process_task(task_id: str, payload: dict) -> None:
    ...
    try:
        ...
        if images:
            result_bytes = await edit_image(images, prompt, aspect_ratio=aspect_ratio)
        else:
            result_bytes = await generate_image(prompt, aspect_ratio=aspect_ratio)
