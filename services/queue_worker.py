    ...
    try:
        ...
        if images:
            result_bytes = await asyncio.wait_for(
                edit_image(images[0], prompt, aspect_ratio=aspect_ratio),
...
async def _process_video_task(task_id: str, payload: dict) -> None:
    """Process a video generation task using WanVideo."""
    telegram_id = payload.get("telegram_id", 0)
    user_id = payload.get("user_id", 0)
    ...
    tariff = payload.get("tariff", "wanvideo_10s")
    request_id = payload.get("request_id", task_id)
    chat_id = payload.get("chat_id", telegram_id)
    is_admin = payload.get("is_admin", False)
...
async def _process_edit_photo_task(task_id: str, payload: dict) -> None:
    """Process a photo editing task with face preservation (IPAdapter)."""
    telegram_id = payload.get("telegram_id", 0)
    user_id = payload.get("user_id", 0)
    chat_id = payload.get("chat_id", telegram_id)
    prompt = payload.get("prompt", "")

    # FIX #1: decode photo from photo_hex instead of reading non-existent photo_bytes
    photo_hex = payload.get("photo_hex", "")
    photo_bytes = None
    if photo_hex:
        try:
            photo_bytes = bytes.fromhex(photo_hex)
        except (ValueError, TypeError):
            logger.error("Failed to decode photo_hex for task %s", task_id)

    if not photo_bytes:
        await _notify_user(chat_id, "❌ Ошибка: не удалось прочитать файл изображения.")
        await release_generation_lock(telegram_id)
        return
    ...
    try:
        ...
        if status == TASK_STATUS_CANCELLED:
            ...
            await _handle_refund(payload, task_id)
            await _notify_user(chat_id, "❌ Редактирование отменено.")
            return

        # FIX #3: call real IPAdapter edit_image (not the old stub)
...
async def _process_animate_photo_task(task_id: str, payload: dict) -> None:
    """Process a photo animation task with WanVideo."""
    telegram_id = payload.get("telegram_id", 0)
    user_id = payload.get("user_id", 0)
    chat_id = payload.get("chat_id", telegram_id)
    ...
    # FIX #4: get real prompt from payload instead of passing empty string
    prompt = payload.get("prompt", "")
    ...
    try:
        ...
        if status == TASK_STATUS_CANCELLED:
            logger.info("Task %s cancelled before processing", task_id)
            await _handle_refund(payload, task_id)
            await _notify_user(chat_id, "❌ Оживление отменено.")
            return

        # FIX #4 + #5: pass real prompt and use WanVideo workflow (via generate_video)
