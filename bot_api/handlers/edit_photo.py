"""Handler for photo editing via ComfyUI (photo + prompt)."""
...
async def start_edit_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Start photo editing flow — triggered by menu button."""
    ...
    await query.edit_message_text(
        ...
        "_После фото я попрошу вас написать что именно изменить._",
        parse_mode="Markdown",
...
async def receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "✅ Фото получено!\n\n"
        "Теперь напишите *что именно изменить*.\n\n"
        "*Примеры:*\n"
...
async def receive_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Receive prompt and enqueue edit task."""
    ...
    if not prompt:
        await update.message.reply_text(
            "⚠️ Пожалуйста, напишите описание изменений.",
            reply_markup=cancel_keyboard(),
        )
        return WAITING_FOR_PROMPT
    ...
    try:
        ...
        await update.message.reply_text(
            f"⏳ *Задача принята!*\n\n"
            f"Промт: _{prompt}_\n\n"
            f"Обрабатываю фото через ComfyUI... Это займёт 30–90 секунд.",
            parse_mode="Markdown",
            reply_markup=back_to_menu_keyboard(),
        )
        ...
        log_exception(exc, trace_id=trace_id, context="edit_photo.receive_prompt")
        await update.message.reply_text(
